...
def load_attacker_model(device):
    attacker_model = GPT2LMHeadModel.from_pretrained('openai-community/gpt2', output_hidden_states = True)
    return attacker_model.to(device)

def process_dataset(tokenizer, text, device, max_length = 256):
...
def load_embeddings(tokenizer, device, target_model, attacker_model):
    ...
    return tm_embeddings.to(device), am_embeddings.to(device)

def train(target_model, attacker_model, inputs, labels, am_embeddings, tm_embeddings, device, tokenizer):
    ...
    target_model_logits = target_model(input_ids = input_ids.unsqueeze(0)).logits
    ...
    with torch.no_grad():
        ...
        original_output = attacker_model(torch.LongTensor(input_ids).unsqueeze(0)).hidden_states[-1]
